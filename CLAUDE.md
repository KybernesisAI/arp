...
| 8 | Mobile Apps (iOS + Android via Expo) | `docs/ARP-phase-8-mobile.md` | ✅ scaffold merged (PR #12 monorepo pointer; app at `github.com/KybernesisAI/arp-mobile`) — public launch deferred post-10 |
...
| **AgentID** | **Identity-first .agent product on top of ARP Cloud** — the current direction. Slices S0–S6 | **`docs/ARP-agentid-plan.md` (canonical; overrides this table where they disagree)** | S0 lander ✅ (PR #150) · S1 revival ✅ · S2 registrar-in-console ✅ (live: `agentid-test.agent`) · S3 identity links ✅ · S4 Eve package + push ✅ (PR #159; `@kybernesis/identity@0.1.0` published; **live two-agent gate ✅ 2026-09-18** on Ian's exe.dev agents `kyber.agent` ↔ `sid.agent`, PRs #167–#170; identity 0.1.1 = `ask_<name>_agent`) · **S5 A2A transport + signed cards ✅ merged + live 2026-09-18** (PR #162 + #163/#164/#165; new `packages/transport-a2a`; RFC-0005; prod identities are exported custody → unsigned cards until S5b `arpc` signing) · **S6a zero-code connect ✅ 2026-09-18** (PR #173 + platform #75; owner pastes a URL, clicks Connect; identity 0.2.0 self-configures) · **S6c identity profile ✅ 2026-09-21** (PRs #215/#216 + platform #77 + kybernesis-admin #20; the name is the source of truth for name/description/picture/accent; served at the name's address; Buzz + control plane pull from it) · **S6d owner account ✅ 2026-09-21** (`/account`: name → owner label default, email one-time-code sign-in from any device via Resend, passkeys/recovery phrase; migration 0018; handoff 2026-09-21) · S6b (tiers + commerce) next |
...
- **Phase 6:** adapter size budgets, DIDComm isolation still holds, each required adapter's conformance test invokes `runAudit` + asserts 11/11 (grew from 8 in Phase 9 slice 9c), LangGraph uses real `@langchain/langgraph`
...
- **Phase 9:** all `@kybernesis/*` packages at `1.0.0` on `latest`, ghcr image signed + tagged, app store builds submitted. **Slice 9c:** `pnpm --filter @kybernesis/arp-cloud-app test` passes the four 429 assertions (registrar-bind 61st, push-register 11th, user-did-json 121st, /onboard 11th); `grep -rn -i 'self.xyz|selfxyz' packages/ tests/ adapters/ apps/` returns only the six intentionally-preserved files (scope YAML + spec-site scrubber + spec-site bundle protective + testkit no-selfxyz probe + its test + audit.ts/types.ts/probes/index.ts mentions of the probe name); testkit `DEFAULT_PROBE_SUITE.length === 11`.
- **Phase 10:** `docs/ARP-phase-10-gap-audit.md` is the starting point — every cloud surface listed as "❌ no UI" or "❌ none" must become "✅ wired" before the phase closes. **Slice 10a:** `POST /api/pairing/invitations` + `POST /api/pairing/accept` exist and pass route tests on the cloud app, logout button renders in `apps/cloud/components/app/AppShell.tsx` + `apps/owner-app`, invitation URL fragment never appears in server access logs (signed payload stays client-side). **Slice 10b:** `/connections`, `/connections/[id]`, `/connections/[id]/audit`, `/connections/[id]/revoke` all render with real data from cloud-db; route tests cover revoke-propagation end-to-end. **Slice 10c:** dashboard pulls real connection counts + agent health + incoming-pairing widget from the DB (not placeholders); `not-found.tsx` + `error.tsx` + `global-error.tsx` render across all three apps; `/support` page exists. **Slice 10d:** `apps/owner-app` consumes `@kybernesis/arp-ui` preset; passkey sign-in option renders; HKDF rotation UI mirrors cloud. **Slice 10e:** `tests/phase-10/*.test.ts` suite green (pair-cloud-cloud, pair-cloud-sidecar, pair-sidecar-sidecar, message-roundtrip, policy-deny, revoke-in-flight, rotate-old-audit-verify); `npx @kybernesis/arp-testkit audit samantha.agent` returns 11/11 against Ian's live sidecar. **Phase-level:** Phase-9 launch checklist (`docs/launch/checklist.md`) remains HELD until all 5 Phase-10 slices merge AND 10e validation returns clean.
...
- **`apps/cloud/tests/*` routes tests mock `next/headers` inline.** Every session-authed route test duplicates the same ~20-line `vi.mock('next/headers', ...)` stub because sharing a testing helper is not wired up. Minor code smell; extract to `apps/cloud/tests/helpers/cookies.ts` at Phase 9 prep cleanup. Added Phase 9 slice 9b. **Closed in slice 9c** — `apps/cloud/tests/helpers/cookies.ts` exposes `installCookieMock()` + `installHeadersMock()`; tenants-route + onboard-route + v2-1-registrar-flow integration now consume it. Remaining duplication (`vi.mock('@/lib/session', ...)` across onboard-route + push-register-route) is small enough to leave alone.
- **Slice 9c rate-limiter is DB-backed, single-region.** `apps/cloud/lib/rate-limit.ts` + `rate_limit_hits` table. Fine for launch traffic — Neon row-level increments are cheap, and we opportunistically sweep expired rows on 1/1000 requests so there's no cron dependency. If we ever scale multi-region, the Vercel regions fan out to a single Neon primary anyway, so the source-of-truth is still one DB. A Redis-backed rate limiter only becomes compelling if the Neon bill becomes rate-limit-dominated (unlikely in v0). Added Phase 9 slice 9c.
- **Owner-scoped testkit probes (`principal-identity-method`, `representation-jwt-signer-binding`) skip without `ProbeContext.ownerLabel`.** `no-selfxyz-prompt` skips without `ProbeContext.registrarSetupUrl`. Local-only audits (phase-5 + phase-6 acceptance) therefore report 11/11 with three skips. CLI plumbing to pass `--owner <label>` and `--registrar-setup-url <url>` to the audit invocation is a slice-9e deliverable. Until then, the three new probes fire only when driven programmatically (e.g. from a nightly job that knows the target's owner label). Added Phase 9 slice 9c.
- **Self.xyz fixture sweep preserved intentional references.** `packages/scope-catalog/scopes/credentials.proof.zk.request.yaml` (scope description legitimately names Self.xyz as a ZK-proof example provider), `apps/spec-site/scripts/port-docs.mjs` (the scrubber), `apps/spec-site/tests/bundle.test.ts` (the protective regex). Do NOT rewrite those in future sweeps. Added Phase 9 slice 9c.
- **WebAuthn rpId is `arp.run`** (the apex domain), not a single surface. A passkey registered from `cloud.arp.run` also signs in on `app.arp.run` and `arp.run` — chosen because the ARP sign-in page moves across surfaces depending on context (marketing, dashboard, registrar redirect). `WEBAUTHN_RP_ID` / `WEBAUTHN_RP_NAME` / `WEBAUTHN_ORIGINS` env vars override in staging. Added Phase 9 slice 9d.
- **HKDF v2 migration is user-initiated; grace is 90 days.** `POST /api/tenants/rotate` requires BOTH signatures (old key + new key) over `arp-rotate-v1:{old}:{new}:{ts}`, clamped to ±5 min. `/u/<uuid>/did.json` dual-publishes verification methods while `v1_deprecated_at + 90 days > now()`, then opportunistically clears the old-DID columns. No forced rotation; v1 users keep working indefinitely until they opt in. Added Phase 9 slice 9d. Closes Phase 8.5 conservative calls #1 (WebAuthn) and #3 (HKDF).
- **Passkey is the AUTHENTICATOR; did:key stays the IDENTITY.** Registering a passkey creates a `user_credentials` row bound to the tenant — the principal DID is untouched. Sign-in via passkey issues a session under `tenant.principal_did` (post-rotation-aware). The WebAuthn credential surface is tenant-scoped + independent from key rotation. Added Phase 9 slice 9d.
- **`apps/cloud/lib/principal-key-browser.ts` dual-stores v1 + v2.** `arp.cloud.principalKey.v1` retained read-only for verifying pre-rotation audit signatures; new accounts write to `arp.cloud.principalKey.v2`. `rotateToV2()` is the only entry point that creates the v2 row from an existing v1 phrase. Ignore: Phase 10+ mobile repo needs the same change; docs-only handoff lives in the slice-9d PR body. Added Phase 9 slice 9d.
...
*Last updated 2026-09-18 on `agentid-s6-connect` (AgentID slice S6a). Vault mirror folder is `Documents/ARP/`.*
