...
export async function loadBadgeData(sldInput: string, opts: { avatar?: string } = {}): Promise<BadgeData> {
  ...
  const avatarUrl = opts.avatar && /^https:\/\/[^\s]+$/i.test(opts.avatar) ? opts.avatar : localAvatar ?? `https://api.dicebear.com/9.x/notionists/png?seed=${encodeURIComponent(sld)}&size=512&backgroundColor=1c1c22`;
  return {
    sld,
