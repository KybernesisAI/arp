...
**Topology:**
```
[Outside agents] ──HTTPS / DIDComm──▶ [cloud.arp.run runtime] ──proxy──▶ [Your agent's API]
                                             │
...
3. Point `kyberbot.agent` A record at `cloud.arp.run`'s IP (automatic via registrar API).
...
# 2. In cloud.arp.run owner app:
#    - Paste the handoff bundle
#    - Enter your OpenClaw agent's API URL + auth token
#    - Click "Provision"
# 3. cloud.arp.run now fronts openclaw-ian.agent, proxies traffic to OpenClaw.
...
<div className="mt-16 border-t border-rule pt-6 font-mono text-kicker uppercase tracking-[0.14em] text-muted">
  Source: <a href="https://github.com/KybernesisAI/arp/blob/main/docs/ARP-installation-and-hosting.md">docs/ARP-installation-and-hosting.md</a>
  {' · '}Ported 2026-04-23
</div>
