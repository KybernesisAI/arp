...
`required_vcs` is a list of opaque VC type identifiers. In v1, the scope catalogue ships examples (prefixed `vc_provider.*` for historical reasons) but any provider can issue VCs; the PDP treats them as strings.
...
<div className="mt-16 border-t border-rule pt-6 font-mono text-kicker uppercase tracking-[0.14em] text-muted">
  Source: <a href="https://github.com/KybernesisAI/arp/blob/main/docs/ARP-architecture.md">docs/ARP-architecture.md</a>
  {' · '}Ported 2026-04-23
</div>
