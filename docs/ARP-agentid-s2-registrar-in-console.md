...
**Progress (2026-09-17, night) — S2 COMPLETE.** PRs #152 (slice), #153 (docs), #154 (per-name records page `/names/<sld>`), #155 (mirror on Vercel + glibc gateway image). Live: migration 0010 on prod ✅; `samantha.agent` re-minted on the mirror ✅; real upstream registration `agentid-test.agent` (order 934, 1 Gem ≈ $0.52) ✅; **mirror live** — `https://samantha.agent.arp.run/.well-known/did.json`, `/representation.jwt`, `/` (profile) and `https://agentid-test.agent.arp.run/.well-known/agent-card.json` all 200 ✅. **Decision change to S2-4 / plan D7:** the mirror is served by the **Vercel cloud app**, not the Railway gateway — Railway caps custom domains per service on the current plan and served its default cert for the wildcard. Middleware `mirror` surface proxies machine paths to the gateway with `?target=<sld>.agent` and renders the profile for everything else; the wildcard domain `*.agent.arp.run` is attached to the `arp-cloud` Vercel project (zone's default `*` ALIAS covers DNS). Gateway: Railway does **not** auto-deploy from GitHub — deploy with `railway up --service arp-cloud-gateway --environment production --detach` from the repo root (needs `railway login`); image switched to `node:24-bookworm-slim` because the alpine build compiled better-sqlite3 from source and the Node-headers download timed out on Railway's builder. **Still open (non-blocking):** reseller flag + wholesale (register response had `reseller_channel: null`); upstream webhook subscribe endpoint 404 (daily cron reconciles); browser purchase smoke by Ian (unit-tested only); testkit `--resolver mirror` flag (S5); price placeholder $29/yr.
