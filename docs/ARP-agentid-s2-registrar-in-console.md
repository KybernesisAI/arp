...
**Progress (2026-09-17, evening):** PR #152 merged + deployed. **Live gate results:** migration 0010 applied to prod Neon ✅; `*.agent CNAME j7y1xavo.up.railway.app` added on the arp.run zone ✅; `samantha.agent` re-minted as a cloud-custody identity on the mirror origin ✅ (profile live at `agent.arp.run/samantha`, shows VERIFIED OWNER from the existing binding); **real upstream registration proven** — `agentid-test.agent` registered via the API key (HTTP 201, domain_id 386, order 934, **1 Gem ≈ $0.52**; the May human-path registrations cost 50 Gems each), recorded in `domain_registrations` (status registered) and minted (profile at `agent.arp.run/agentid-test`, OWNER PENDING until the browser signs the proof). Env set in Vercel prod+preview: `HEADLESS_API_KEY`, `ARP_CLOUD_KEY_ENCRYPTION_KEY`, `CRON_SECRET`, `HEADLESS_WEBHOOK_SECRET`. **Still open:** (1) ⛔ Railway wildcard custom domain `*.agent.arp.run` on the gateway service — needs `railway login` (CLI session expired); until then `https://<sld>.agent.arp.run/*` fails TLS; (2) upstream `POST /api/v1/webhooks/subscribe` returns **404** (documented, not implemented) → the daily reconcile cron is the sync path; (3) `reseller_channel` came back `null` on the register response → account not flagged reseller yet (§5-2); (4) the browser purchase flow (Stripe test card → webhook → fulfilment) has only been exercised in unit tests; Ian to run one from the dashboard; (5) T12 docs (this edit). Remaining T-items: T11 browser-side verify for samantha (Finish setup button), T12.
