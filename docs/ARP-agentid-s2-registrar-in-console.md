...
**Progress (2026-09-17):** T1–T10 ✅ built + unit-tested (registry client, migration 0010, cloud key custody + mintIdentity, search/checkout/fulfilment, self-hosted owner binding, mirror host in gateway, profile page, console names UI, upstream webhook + daily reconcile). Remaining: **T11** re-mint samantha.agent (after deploy + mirror DNS), **T12** docs. **Ops checklist before the live gate:** (a) push + merge PR; (b) apply `packages/cloud-db/migrations/0010_agentid_registrations.sql` to prod Neon via `migrate-once.mjs`; (c) DNS `*.agent CNAME j7y1xavo.up.railway.app` on the arp.run zone (Vercel DNS) + add wildcard custom domain `*.agent.arp.run` to the Railway gateway service (`railway login` needed); (d) subscribe the upstream webhook to `https://cloud.arp.run/api/webhooks/headless` with `HEADLESS_WEBHOOK_SECRET`; (e) env already set in Vercel prod+preview: `HEADLESS_API_KEY`, `ARP_CLOUD_KEY_ENCRYPTION_KEY`, `CRON_SECRET`, `HEADLESS_WEBHOOK_SECRET`. **Decision change vs §1:** S2-5 owner binding is now fully self-hosted (the upstream bind endpoint requires a browser login, not an API key); the proof is served from the mirror at `/representation.jwt`, which the DID document already points at. Upstream `profile._arp` sync becomes an ask (§5-4). **Measured:** the API key registers `.agent` at the agent rate (quote: 1 Gem ≈ $0.52/yr), not the $26 human retail; the account already owns 10 names (atlas, arcana, hermes, kitt, nova, mythos, athena, ghost, samantha, ian.chatbot).
