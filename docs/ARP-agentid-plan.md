...
| **S4** | Eve thin package + push delivery — ARP side ✅ live 2026-09-18 (PR #159: push delivery to Eve/generic runtimes with JWKS-verified ES256 tokens, replies signed by the cloud-held key, agent-API connections/send, attach/detach runtime + panel, freshness gate). Platform side ✅ published `@kybernesis/identity@0.1.0` (npm + registry, platform PR #73). S3b ✅ (kybernesis-admin PR #19, deployed). Open: live two-agent gate. Brief: `docs/ARP-agentid-s4-eve-package-push.md` | `@kybernesis/identity` in `~/platform` (channel `arpAuth()` in the dispatch auth walk; `arpPeers()` dynamic tools from `/api/connections`; extension with contact skill; registry item); ARP side: cloud-bridge Eve adapter (Eve session API) + **push delivery mode** in cloud-runtime; handoff shape unification | two Eve agents in two tenants pair via cloud.arp.run, exchange a message, and hit a visible policy denial; no daemon involved |
| **S5** | A2A transport + signed card + mirror — **started 2026-09-18**, brief `docs/ARP-agentid-s5-a2a-transport.md`: `/.well-known/agent-card.json` becomes the A2A v1.0 card (ARP card moves to `arp-card.json` + rides as extension `https://arp.run/ext/arp/v1`), cards signed with the identity key (EdDSA, `jku` = mirror JWKS), mirror `/a2a` JSON-RPC endpoint with the Connection Token as bearer → `dispatchInbound`, `transport-a2a` for outbound, testkit `a2a-card` probe + `--resolver mirror` | `packages/transport-a2a` implementing `Transport`; ARP as A2A extension; signed agent card (JWS/JCS) binding DID + `.agent` + mirror host; `supported_scopes` actually populated; ICANN mirror host per name; testkit probes updated | a stock A2A client resolves and addresses a `.agent` identity via the mirror with zero custom code |
