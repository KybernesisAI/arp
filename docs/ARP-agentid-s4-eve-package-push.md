...
**Parent:** `docs/ARP-agentid-plan.md` §6 (S4), decisions D6 + hardening gaps in §5. **Status (2026-09-18):** ARP side ✅ merged + live (PR #159): migration 0012 on prod, gateway redeployed with `ARP_CLOUD_PUSH_SIGNING_JWK` / sealing key / issuer → `https://gateway.arp.run/.well-known/jwks.json` live, agent-API answering, attach panel on `/names/<sld>`. Platform side ✅ **published**: `@kybernesis/identity@0.1.0` on npm (`latest`), platform PR #73 merged to master, `registry.kybernesis.ai/r/identity.json` live (`eve add @kybernesis/identity`). **E3 / S3b ✅** merged (kybernesis-admin PR #19) + deployed to agent.kybernesis.ai ("Link .agent name" next to Mint credential). **Live gate (2026-09-18) ✅ run against Ian's self-hosted Eve agents on exe.dev** — Kyber (`kyber.exe.xyz`) as `agentid-test.agent` and Sid (`sid-agent.exe.xyz`) as `nova.agent` (stand-in until the pending `sid.agent` purchase completes), both in Ian's tenant. Both VMs got `@kybernesis/identity`, `agent/channels/arp.ts`, `agent/tools/arp-peers.ts`, `agent/instructions/arp.md`, `arpAuth()` in the eve channel's `extraAuth`, and the four env vars; names re-provisioned into hosted custody, runtimes attached through `POST /api/agents/[did]/runtime`, paired in the console with `messaging.relay.to_principal` both ways. Results: (a) Sid → Kyber relay through the agent API returned Kyber's reply ("Hi Sid, I received your message.") via push delivery — no socket, no daemon; Kyber's turn lists `ask_nova` among its tools. The tool-*invoked* leg (Kyber calling `ask_nova` and Sid answering) is blocked only by Sid's expired Claude-subscription OAuth on the sid VM (push reached Sid and authenticated; the model backend failed). (b) an out-of-scope `calendar.events.read` request → `pdp_deny` in Sid's audit chain + `403 denied` to the caller. (c) no daemon on either side. (d) revoking the connection in the console emptied the peer list and refused the next send immediately; re-paired afterwards (`conn_g8ZKMnRgWc4EaJ5p`). Four defects found + fixed on the way (PRs #167–#170): no path to re-provision an exported name into hosted custody; DID document schema rejected the S3 link service types (pairing failed for any name with an attached runtime); the agent API could not see same-tenant connections from the initiating side; a PDP deny surfaced to the agent API as a timed-out reply.
