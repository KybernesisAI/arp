...
Edit `src/hooks.ts` (or `hooks.py`) to connect each integration point to the framework extension surface from Step 2. Use the 30-line template in the guide's §12 as the starting shape; expand per framework idioms.
...
pnpm install
pnpm test
pnpm --filter adapters/<slug> test:conformance
```
