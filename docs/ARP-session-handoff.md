...
>
> **2026-09-18 lock-in (AgentID S6a done; side quest in progress).** S6a zero-code connect is merged + live (ARP #173, platform #75; brief `docs/ARP-agentid-s6-zero-code-connect.md`). Kyber + Sid run with zero ARP env vars — identity in `<app>/.eve/arp-identity.json`, connected from the console in one click. Pending on Ian: `npm publish` of `@kybernesis/identity@0.2.0` + `@kybernesis/create@0.16.0`; then switch both VMs from the tarball to the registry version. **Resume point:** S6b (tiers + commerce, pick-from-platform connect, hosted secret store). Side quest ✅: interactive 3D ID badge at `agent.arp.run/badge` (`AgentBadge` + `loadBadgeData`, reusable on any agent page) and a new AgentID landing page at `/lander` (black hero + badge + name search; black-on-white sections). Swap `/lander` in for the agent.arp.run root when Ian says so.
...
14. **Phase 9 slice 9c — Rate limiting + 3 testkit probes + Self.xyz fixture sweep + cookie helper.** Four deliverables in a single branch. (1) DB-backed fixed-window rate limiter at `apps/cloud/lib/rate-limit.ts` + Drizzle schema `rate_limit_hits` + migration `0003_phase_9c_rate_limits.sql`. Applied to `GET /onboard` (10/min IP + 100/hour IP), `POST /internal/registrar/bind` (60/min IP + 600/hour IP), `POST /api/push/register` (10/min tenant), `GET /u/<uuid>/did.json` (120/min IP). 429 + `Retry-After` header. Atomic increment via `ON CONFLICT DO UPDATE SET hits = hits + 1`. Opportunistic sweep of expired windows on 1/1000 requests — zero cron dependency. (2) Three v2.1 §6 probes in `@kybernesis/arp-testkit`: `principal-identity-method` (resolves `_principal.<owner>.<domain>` TXT `did=` value; both did:web + did:key accepted), `no-selfxyz-prompt` (warn-only HTML grep of registrar's ARP-setup URL), `representation-jwt-signer-binding` (fetches representation.jwt, decodes header/payload, resolves `iss`, verifies Ed25519 sig against kid-matched VM). `DEFAULT_PROBE_SUITE` grew 8 → 11; owner-scoped probes skip on local runs (no `ownerLabel` in context). (3) Self.xyz test-fixture sweep: 52 files changed, `did:web:ian.self.xyz` → `did:web:ian.example.agent` etc. across `packages/*/tests`, `adapters/*/tests`, `apps/*`, `tests/phase-*`. Preserved six intentional references (scope YAML, spec-site scrubber, spec-site protective assertion, testkit no-selfxyz probe + its test + suite mentions). (4) `apps/cloud/tests/helpers/cookies.ts`: `installCookieMock()` + `installHeadersMock()` — dedupes `next/headers` mock; consumed by `tenants-route.test.ts` + `onboard-route.test.ts` + `tests/phase-9/v2-1-registrar-flow.test.ts`. Acceptance tests that hard-coded `summary.total === 8` updated to 11 (phase-5 + phase-6). 4 commits, all gates green on cold cache. **Post-merge actions required by Ian:** apply migration `0003_phase_9c_rate_limits.sql` to production Neon via `packages/cloud-db/migrate-once.mjs` (no new env vars).

15. **Phase 9 slice 9d — WebAuthn + HKDF rotation.** 6 commits. WebAuthn register + sign-in via `@simplewebauthn/server@13.3` (four new `/api/webauthn/*` routes), user_credentials + webauthn_challenges tables (migration 0004), `apps/cloud/lib/principal-key-passkey.ts` client helpers, `/cloud/login` rewrite with passkey CTA + did:key recovery fallback, `MigrateToPasskeyBanner` on dashboard. HKDF v2 seed derivation (@noble/hashes HKDF-SHA256, salt=arp-v2), `POST /api/tenants/rotate` (double-sig over `arp-rotate-v1:{old}:{new}:{ts}`, ±5-min clamp, unique-violation → 409, idempotent replay), `/u/<uuid>/did.json` dual-publishes during 90-day grace + opportunistic cleanup, RFC-0005. **rpId: arp.run** (apex) so passkeys registered on cloud.arp.run work on app.arp.run + arp.run. Migration 0004 applied to prod Neon; `WEBAUTHN_RP_ID` + `WEBAUTHN_RP_NAME` + `WEBAUTHN_ORIGINS` set in prod env + redeploy fired so they load. All 4 WebAuthn endpoints verified live.

16. **Phase 9 slice 9e — Launch polish.** 7 commits + 3 post-merge hotfixes (status.arp.run host-rewrite, Date.now freeze on rate-limit burst tests, legal DRAFT prefix cleanup). Ships: status page at `apps/spec-site/app/status/` (static, 3 service groups), `ops/incident-runbook.md` + `ops/on-call-rotation.md`, `docs/launch/checklist.md` (8 sections, the human-in-loop launch runbook), hello-world launch post (~1500 words, noindex), HN summary + demo script + FAQ, `/legal/{terms,privacy,dpa}` skeleton pages + `LegalDraftBanner`, `/dashboard/settings` with passkeys list + inline rename + delete (last-credential guard), identity rotation panel, recovery-phrase reveal gated on typing `SHOW ME`, `PATCH`/`DELETE /api/webauthn/credentials/:id`, mobile-repo handoff at `docs/ARP-phase-9-slice-9e-mobile-followup.md`. `status.arp.run` attached to arp-spec-site Vercel project via the workflow; middleware rewrites bare `status.arp.run/` → `/status`. 32 new cloud-app tests. Post-launch-week-1 path: per `docs/launch/status-probe-options.md`, Option 1 (Vercel Cron + testkit + Neon) selected for live probes.

17. **Post-9e launch-prep work (held).** PR #35 (`chore(release): v1.0.0`) bumps all 25 `@kybernesis/arp-*` packages to 1.0.0 — CI green, HELD for launch day. Docker sidecar dry-run clean, image 69.4 MB (< 300 MB budget). Legal + hello-world post unblocked (noindex removed, DRAFT banners stripped, `LegalDraftBanner` component deleted) — PRs #37 + #38 merged, pages live at `cloud.arp.run/legal/*`. Six `[TODO: counsel]` placeholders remain in legal pages (jurisdiction, subprocessor list, SOC 2 commitment, retention window) — specific business decisions pending.

18. **Phase 10 kickoff — product completion.** Gap audit caught that cloud app is missing pairing UI, audit log viewer, revocation UI, logout button, error pages, support page, real-data dashboard widgets, across cloud + owner-app + sidecar. Phase 9 launch checklist (`docs/launch/checklist.md`) is **HELD** pending Phase 10. Scope: `docs/ARP-phase-10-product-completion.md` (5 slices — 10a pairing + logout, 10b audit + revoke, 10c dashboard + polish, 10d owner-app parity with cloud, 10e e2e tests + real-world samantha.agent validation). Coordinating Claude produced this kickoff doc + gap audit + CLAUDE.md/handoff bumps in one PR. Ian's `samantha.agent` + DNS control unblock real-world validation without Headless's registrar dependency.

### Phases remaining
- **Phase 10 — Product completion.** 5 slices as above. ~6–10 days focused work.
- **Phase 9 launch checklist resume** — after Phase 10 merges, work through `docs/launch/checklist.md`: Stripe sandbox→live, SSO "All Deployments" toggle, npm publish at 1.0.0, GHCR image push, Headless 11/11 sign-off, HN post.
- **Mobile public launch.** iOS App Store + Google Play Store submissions in `github.com/KybernesisAI/arp-mobile`. Prereq: Phase 10 + launch checklist. Apple Dev account + EAS credentials + Play Console setup required. Separate track, post-launch.
...
- **Lander family (2026-09-20, PRs #204–#206):** monochrome "works with" logo row (`apps/cloud/app/lander/Logos.tsx`: Eve, OpenClaw, Hermes Agent pixel-traced, LangGraph, Grok Bot from x.ai/bot; heights tuned to one optical cap height). Shared lander chrome + helpers in `apps/cloud/app/lander/ui.tsx`; generalised `BadgeHero`. **Public agent profile `agent.arp.run/<sld>` redesigned in the lander style** (badge hero, identity-record bento, verified links, reach-by-name records, claim CTA); the old Swiss `agentid/layout.tsx` now lives in the `agentid/(marketing)` route group and wraps only the marketing index. Known pre-existing quirk: unknown names render the 404 page with HTTP 200 through the host rewrite.

## 2026-09-20 — Kyber + Sid fully integrated; Cedar policy layer proven live

- **Owner verification** = `/names/<sld>` → "Verify ownership" (browser signs the owner proof with the account key; label = how the owner is shown publicly). The button was hidden for `active` registrations recorded directly (PR #209 fixed; now asks for a label). `kyber.agent` + `sid.agent` owner = `ian`.
- **Kyber links (all verified):** runtime `kyber.exe.xyz/eve/v1/arp`; **Buzz** `npub1zvnae95q6qtq4avht747jzaf657k57fl08q0kf9qk53g5p7qj0zq5xwzan` (signed with the key in `~/.kybernesis/buzz-kyber.json` on the host via `nostr-tools/pure`; NIP-05 live at `kyber.agent.arp.run/.well-known/nostr.json?name=_`; DID doc `alsoKnownAs`); **control plane** `agent:55627c0e-34a9-4405-9a45-2ab20b91f14b/Kyber` (statement minted from agent.kybernesis.ai → "Link .agent name", pasted to the console). **Sid links:** runtime + control plane `…/sid` (no Buzz by design).
- **Pairing** `conn_WjrVWbVEUe5g879J` (30 days, to 2026-10-20): Kyber grants Sid `messaging.relay.to_principal` + `work.status.read`; Sid grants Kyber relay + `work.status.read` + `work.projects.list`. Old `conn_b6CsGsqp04uFnHoa` had **expired after the 1-day default** and was revoked.
- **`@kybernesis/identity@0.3.0`** (platform PR #76, merged; **not yet on npm** — both VMs run the tarball `file:../kybernesis-identity-0.3.0.tgz`): `ask_<name>_agent` takes `intent` = message | status | projects | files | calendar → ARP scope ids. Gateway prompt names the request kind (PR #210).
- **Policy test (direct, from Sid's host):** relay → 200 reply; `work.status.read` → 200 ("Available and idle…"); `work.projects.list` → 403 policy_denied; `calendar.events.modify` → 403. **Tool-invoked:** Kyber → Sid asked Sid to call `ask_kyber_agent` with three intents; Sid reported status allowed, projects "kyber declined: policy_denied", message allowed. Both audit chains hold every hop (Kyber 8 entries, Sid 5, all hashed).
- **Fixes shipped from the test (PR #211):** expired connections were still listed + dispatchable → now denied (`connection_expired`) and hidden; Cedar deny with no matching policy now audits `policy_denied: <action> on <Resource::id>`; pairing default 30 days. Consent headline on `/pair/accept` reads "<audience> wants to connect with <subject>" (spec-example wording, written from the granting owner's side) — confusing for the accepting side; copy follow-up, not changed.
- Known cosmetic: connection obligations list `rate_limit` twice (once per direction's amendment).

## 2026-09-21 — S6c identity profile live (three repos)

- **ARP** (#215, #216): migration 0016 on prod; `PUT /api/names/[sld]/profile`; ProfilePanel on `/names/<sld>`; gateway `GET /avatar.png` + `/.well-known/agent-profile.json`; DID doc `AgentProfile` service; card `iconUrl`; mint/rebuild/gift carry the profile; badge picture precedence identity → checked-in PNG → placeholder. Also #213 (push agents report online: last-seen on delivery + health probe) and #214 (kyber/sid portraits).
- **Platform** (#77, merged, **not yet on npm**): `@kybernesis/buzz@0.9.12` `kybernesis-buzz profile --from-agentid` — run on Kyber's host 2026-09-21 → kind-0 with picture + `nip05: _@kyber.agent` accepted by both relays. Kyber is back on npm buzz 0.9.9 (a hand-built 0.9.12 tarball lacked the `eve extension build` step → `.js` not `.mjs`, broke eve build; the package's own `prepack` builds correctly). Identity 0.3.0 was already on npm (published 2026-09-20 18:20 UTC); both VMs now run it from the registry.
- **Control plane** (kybernesis-admin #20, merged → Vercel deploys + migrates): `agent_ref.agentid_did`, `src/lib/agentid-profile.ts` (two-way verified, cached), `/api/me/agents` returns `identity` + identity-filled `profile` defaults. Kyber + Sid need their link re-minted once from the admin UI ("Link .agent name") so `agentid_did` is set — the column is new.
- Kyber: picture + accent `#10b981` + description set; Sid: picture + `#22d3ee`. Profile JSON verified live; card signed with iconUrl.
- buzz 0.9.12 is on npm (published 18:53 UTC, after the #77 merge at 18:47 — it carries `--from-agentid`). **Kyber cannot install 0.9.10+**: those declare peer eve `>=0.51` and Kyber runs eve 0.49 (that is why he is pinned to 0.9.9; installing 0.9.12 made `eve build` reject the buzz extension). The 0.9.12 CLI is installed in his project under an alias (`buzz-cli@npm:@kybernesis/buzz@0.9.12`, so it resolves eve/ai from the project while the extension stays on 0.9.9; npx cannot be used — peers cascade). Re-sync his Buzz profile with `cd ~/kyber && set -a && . ./.env.local && set +a && node node_modules/buzz-cli/dist/cli.js profile --from-agentid` (verified 2026-09-21; eve build unaffected). Sid runs eve 0.51.1 and can take 0.9.12 normally when he gets Buzz.

## 2026-09-21 — dashboard redesign (PR #225)

- `/dashboard` rebuilt in the lander language inside a new `ConsoleShell` (black bar: Agents · Connections · Pair · Billing · Docs · Log out). The old page listed the same agents three times (agents table = "Agents", `domain_registrations` = "Your names", `registrar_bindings` = ".agent domains"); now every domain the account touches is **one card** merging identity + purchase + owner proof, linking to `/names/<sld>`. States: Online / Offline / Not connected / Set up needed / Not set up here (binding-only, e.g. hermes.agent from the Headless flow → Provision / Unbind). Pairing requests as cards on top; claim-a-name (`ClaimName.tsx`), recent activity, plan tile. Dropped: raw principal DID, skills matrix (`SkillsSection` code path no longer rendered; `ClaimNamePanel`, `SelfTestConnectionButton`, `DeleteAgentButton` are now unused by the dashboard).
- Hero emerald burst (#223) reverted in #224 — Ian did not like it.
- Other console pages (`/names/<sld>`, `/connections`, `/pair`, `/billing`) still use the Swiss `AppShell`; moving them over is the natural next side-quest.
- **All 13 of Ian's .agent names are now AgentID agents in his tenant** (Ian: "all the .agent domains I have, even the old ones I originally bought on headless should be here"). Backfilled `domain_registrations` rows (status active, price 0, expiry from Headless `my-domains`) for the nine names bought on Headless before the console sold names (atlas, arcana, hermes, mythos, nova, samantha, kitt, athena, ghost); minted hosted identities for hermes, kitt, athena, ghost; owner-verified kitt, athena, ghost as `ian` (older ones keep label `owner` from the v2.1 flow). Dashboard (#227): identity-less names get a lander-style "Create identity" button (old Swiss Provision/Unbind gone). A pending pairing request (Atlas → Lilly, relay only, 30 days) is left open so the "Pairing requests" cards show; cancel it from the dashboard when done.
- **Console fully on the lander language (PR #230, #231):** every signed-in page (`/names/<sld>`, `/connections` + detail/audit/edit/revoke, `/pair` + accept, `/billing`, `/gift`, `/dashboard/settings`, `/agent/<did>`) now renders in `ConsoleShell` with `ConsoleHead` (drop-in for PlateHead). `apps/cloud/app/console-theme.css` (scoped to `.lander`) re-themes the Swiss primitives by token — white paper, zinc rules, Inter display, black accent, pill buttons/badges, rounded cards/inputs — so pages needed no markup rewrite. Marketing surfaces (arp.run, cloud.arp.run root, agentid marketing index) still Swiss. Dates on console surfaces are MM-DD-YYYY (`mdy()` in dashboard/page.tsx); dashboard renewal date pinned bottom-right of each card. Owner labels on all of Ian's names are `ian`.
- **Connections in plain language (PR #233):** `apps/cloud/lib/connection-view.ts` maps each Cedar permit to the catalog label via `cedar_template` (action verb + resource type) and obligations to sentences; `/connections` = cards "a.agent ↔ b.agent" with "x may …" per side; detail = two permission cards + conditions + dates + Message log / Change permissions / Pause / End. No `did:web:` on any console surface the owner reads.
- **Pair page for owners (PR #235/#236):** the other agent is typed/shown as a plain name (`samantha.agent` or `samantha`); the identifier is derived and never displayed; helper text in words; your-agent dropdown shows names; after generating, the link + Copy link + a **QR code** (client-side `qrcode`) of the same link; expiry MM-DD-YYYY; accept screen shows names, no connection id. Rule going forward: no `did:web:` / identifiers on any owner-facing surface.
- **Short invitation links (PR #238, migration 0017 on prod):** `pairing_invitations.short_token` (128-bit, base64url); links are `https://cloud.arp.run/i#<token>` (~50 chars). Token stays in the fragment (no access-log exposure, same as the payload before); `/i` → `POST /api/pairing/invitations/resolve {token}` (public, 60/min/IP, pending-only) → `location.replace('/pair/accept#<payload>')`. Long links still work; dashboard Copy link prefers the short one; rows minted before #238 have no token. QR is now sparse; click/Enlarge → modal (1024px render, Download PNG `pair-<name>.png`).

## 2026-09-21 — S6d owner account: name, email sign-in, one place for the key

- **Where the account lives:** `/account` (ConsoleShell nav: Agents · Connections · Pair · Billing · **Account**). `/dashboard/settings` redirects there. Sections: **Your name** (`tenants.display_name`; shown as owner on public pages and offered as the owner label by "Verify ownership" — slugified, e.g. "Ian Borders" → `ian-borders`), **Email** (add/change with a 6-digit code; verified state), an "about this account" strip (names count, plan, key location, sign-in methods), then the existing passkeys / key rotation / recovery-phrase sections (`SettingsSections`).
- **How "ian" became the owner label:** it is typed in the "Shown as" field of "Verify ownership" on `/names/<sld>` (default was the literal `owner`, and older Headless-flow names carried `owner` from the v2.1 flow; relabelled to `ian` on 2026-09-21). Now the default is the account name.
- **Email sign-in (any device):** `/cloud/login` has a first panel "Email me a code" → `POST /api/auth/email/start {email}` (always answers "if that address has an account, a code is on its way"; rate-limited 10/10 min per IP, 5/10 min per address) → `POST /api/auth/email/verify {email, code}` → session for the tenant that owns the address (same `setSession` as the key path; nonce `email:<ts>`). Codes: 6 digits, SHA-256(email\ncode) at rest, 10-minute TTL, single use, 5 wrong tries lock, a new code retires the previous one. Account-side: `POST /api/account/email/start` (409 if another account has the address) + `POST /api/account/email/confirm` (code must have been issued to this tenant). `GET/PATCH /api/account` (name). Migration **0018** (`tenants.email` unique lower-index, `tenants.email_verified_at`, table `login_codes`) applied to prod 2026-09-21.
- **Mail:** `apps/cloud/lib/email.ts` posts to Resend over fetch (no SDK). From `AgentID <sign-in@notifications.kybernesis.ai>` (the only verified Resend domain; override `EMAIL_FROM`). `RESEND_API_KEY` set on Vercel `arp-cloud` (Production + Preview) as a Secret, created 2026-09-21 in Resend as key `arp-cloud` (sending-only, domain-scoped to notifications.kybernesis.ai). **Ian rotates it before go-live.** Without the key the app logs the message and continues (tests, local dev).
- **The limit to say out loud:** an email session opens the dashboard, connections, billing, and lets you read everything. Anything that needs the account **key** — verifying ownership of a name, approving a pairing, the recovery phrase — still needs the key on that device (recovery phrase import on the login page, or a passkey registered from a device that had the key). A phone signed in by email can watch; it cannot sign. Passkey-on-first-email-login is the natural next step (register a passkey from an email session → that device can then sign).
- Tests: `apps/cloud/tests/login-codes.test.ts` (8), `apps/cloud/tests/account-routes.test.ts` (5, full cold-device round trip).
- **Live-verified 2026-09-21 (PRs #243, #244):** Ian's account name set to `ian`; `ian@darkstarvc.com` added + verified from `/account`; then from a cold curl session: start → code (read from the Resend "Emails" log, which shows sent subjects — handy when the inbox is not reachable) → verify → `GET /api/account` 200 (name ian, email verified, 1 passkey) → `/dashboard` 200 → the same code a second time is refused. The first Resend key `arp-cloud` was pasted wrong (Resend answered "API key is invalid"); the live key is **`arp-cloud-2`** (sending-only, notifications.kybernesis.ai) — `arp-cloud` is unused and can be deleted in Resend. The rotation block under `/account` is now "Account key" / "Key fingerprint" / "Upgrade account key" (no identifier vocabulary).
- **Signed-out pages in the lander design (PR #246):** `/cloud/login` (three cards: Email code · Passkey · Recovery phrase; advanced external-signer collapsed) and `/onboarding` (three-step rail, 12-word grid) render in a new `components/app/AuthShell.tsx`. The Swiss marketing pages moved to `app/cloud/(marketing)/` (URLs unchanged) so login is no longer wrapped by the marketing layout. Form primitives (`PrimaryButton`, `TextInput`, …) live in `app/lander/ui.tsx`. Still Swiss: `/onboard` (the registrar-redirect flow from the v2.1 spec) and the marketing pages themselves (`/cloud`, `/cloud/features`, `/cloud/pricing`, arp.run).
- **Key on a second device (PR #248):** Ian signed in by email on another computer and "Change permissions" failed with a `did:key` mismatch — the forms called `getOrCreatePrincipalKey()`, which minted a stray key. Now `requirePrincipalKey(sessionDid)` never mints; `components/app/UnlockKey.tsx` (+ `useDeviceKey`) shows "This device does not have your account key yet" with a 12-word box on change-permissions, pair, approve and verify-ownership; `unlockKeyFromPhrase()` keeps only the derivation matching the account and replaces any stray key. After unlocking once, the device signs like the original. `tests/device-key.test.ts`. Natural next step: let an email session register a passkey (then a passkey could also unlock signing — needs the key to be wrapped/synced, which it is not today).
- **Account page device-aware (PR #250):** on a device without the account key, `/account` shows the Unlock card instead of "Reveal recovery phrase" / "Upgrade account key" (Ian hit `NO_V1_KEY_TO_ROTATE` + "no recovery phrase available" on the second computer). `exportRecoveryPhrase()` now rebuilds the 12 words from a v1 key when they were never stored (v1 seed = entropy doubled). Ian's main Chrome holds a v1 key + phrase; the second computer needs that phrase once.

## 2026-09-21 — device link shipped; gateway lost its database for ~3 h

- **Add another device (PR #252, migration 0019 `device_links` on prod):** a device signed in by email gets the account key from a device that has it. New device: unlock card → "From a device that has it" → 6-digit code (ECDH P-256 receiver key registered via `POST /api/account/device-link`), polls `GET …/:id`. Device with the key: Account → "Add another device" → code → `POST …/claim` → seals key + phrase (ECDH → HKDF-SHA256 → AES-256-GCM, `lib/device-link-crypto.ts`) → `POST …/deliver`. Ciphertext handed out once, then wiped; same tenant only; 10 min; 5 wrong codes lock. Used live: Ian's MacBook (out of office, investor demo) got the key from the Mac mini — the sender side was run from the Mac mini's Chrome via JS (hidden automation tab does not hydrate `/account`; see memory note). `exportStoredKeyFor` / `installTransferredKey` in `principal-key-browser.ts`.
- **Gateway outage:** from at least 11:47 UTC the Railway gateway logged `agent_lookup_failed err="Error connecting to database: fetch failed"` (ETIMEDOUT to Neon HTTP) on every request while `/health` stayed 200 → every mirror host answered 404 (`avatar.png`, `agent-profile.json`, `did.json`, cards), so dashboard pictures and public profiles broke. The database was reachable from Vercel and from this Mac. Fixed 14:41 UTC by redeploying the container (`railway up --service arp-cloud-gateway --environment production --detach` from the repo root; `railway redeploy` is blocked by the auto-mode classifier). **Follow-up (not done):** make `/health` run a 2 s DB probe and return 503, and exit the process after N consecutive DB failures so Railway's restart policy recovers it without a human; also a synthetic check on `kyber.agent.arp.run/.well-known/did.json` from the status page.
