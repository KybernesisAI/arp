...
- buzz 0.9.12 is on npm (published 18:53 UTC, after the #77 merge at 18:47 — it carries `--from-agentid`). **Kyber cannot install 0.9.10+**: those declare peer eve `>=0.51` and Kyber runs eve 0.49 (that is why he is pinned to 0.9.9; installing 0.9.12 made `eve build` reject the buzz extension). Re-sync his Buzz profile with `cd ~/kyber && set -a && . ./.env.local && set +a && npx -y -p @kybernesis/buzz@0.9.12 -p eve@0.49.0 kybernesis-buzz profile --from-agentid` (eve is a peer of @kybernesis/enterprise; npx does not install peers). Sid runs eve 0.51.1 and can take 0.9.12 normally when he gets Buzz.
