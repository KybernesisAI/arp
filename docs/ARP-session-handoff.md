...
>
> **2026-09-18 lock-in (AgentID S6a done; side quest in progress).** S6a zero-code connect is merged + live (ARP #173, platform #75; brief `docs/ARP-agentid-s6-zero-code-connect.md`). Kyber + Sid run with zero ARP env vars — identity in `<app>/.eve/arp-identity.json`, connected from the console in one click. Pending on Ian: `npm publish` of `@kybernesis/identity@0.2.0` + `@kybernesis/create@0.16.0`; then switch both VMs from the tarball to the registry version. **Resume point:** S6b (tiers + commerce, pick-from-platform connect, hosted secret store). Side quest started: interactive 3D ID badge at `agent.arp.run/badge` (`apps/cloud/app/agentid/badge/*`), to be lifted onto the lander.
