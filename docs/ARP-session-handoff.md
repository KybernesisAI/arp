...
- **Platform** (#77, merged, **not yet on npm**): `@kybernesis/buzz@0.9.12` `kybernesis-buzz profile --from-agentid` — run on Kyber's host 2026-09-21 → kind-0 with picture + `nip05: _@kyber.agent` accepted by both relays. Kyber runs it from the tarball `file:../kybernesis-buzz-0.9.12.tgz`. Identity 0.3.0 (intents) is also merged and unpublished; both VMs on tarballs.
...
- Publish (Ian): `cd ~/platform/packages/identity && npm publish --access public` · `cd ~/platform/packages/buzz && npm publish --access public`; then on both VMs `npm install @kybernesis/identity@0.3.0` and on Kyber `npm install @kybernesis/buzz@0.9.12` (`--legacy-peer-deps` on Kyber).
