...
/**
 * ARP design-system Tailwind preset — Swiss / editorial tokens.
 *
 * Every token maps to a row in `docs/ARP-design-system.md`. Consumer apps
 * attach this preset in their own `tailwind.config.ts`:
 *
 *   import arpPreset from '@kybernesis/arp-ui/tailwind-preset';
 *   export default { presets: [arpPreset], content: [...] };
 *
 * Paper/ink neutrals reference CSS vars defined in `styles/tokens.css` so a
 * single `[data-theme="dark"]` override flips the whole surface. Signal
 * colors (blue / red / yellow / green) are direct hex because the reference
 * keeps them identical across themes.
 */
